<template>
  <Card>
    ...
    <!-- <div>当前房源置顶中，剩余时长：5天。指定关键词：学院、</div> -->
    <Row>
      <Col>
      <Form ref="form" :model="form" :rules="ruleValidate" :label-width="120">
        <!-- <FormItem label="楼盘名称" v-if="!isDetail" prop="newcode">
        <Select v-model="form.newcode" filterable remote clearable :remote-method="remoteMethod" :loading="ncLoading">
            <Option v-for="(option, index) in ncOptions" :value="option.newcode" :key="index">{{option.projname}}</Option>
          </Select>
        <span>请从列表中选取楼盘！如楼盘不存在，请联系客服创建</span>
      </FormItem> -->
        <Row>
          <Col span="10">
          <FormItem label="楼盘名称">
        ...
        <FormItem label="户型" required>
          <Row>
            ...
            <Col span="1" style="text-align: center">阳台</Col>
          </Row>
        </FormItem>
        ...
        <FormItem label="房源描述" prop="houseDesc">
          <Row>
            <Col span="10">
            <Input v-model="form.houseDesc" type="textarea" :autosize="{minRows: 8,maxRows: 10}"></Input>
            </Col>
          </Row>
        </FormItem>
        <FormItem>
        ...
        <FormItem label="户型图">
          ...
          <Row>
            <div class="housePicList">
              <div class="housePicListItem" v-for="(item,index) in hxImgList" v-if="item.imageType == 4  && item.isDel !=1" :key="index">
          ...
          <Row>
            <div class="housePicList">
              <div class="housePicListItem" v-for="(item,index) in snImgList" v-if="item.isDel !=1" v-dragging="{ item: item, list: snImgList, group: 'sn', key: index }" :key="index">
                <!-- v-dragging="{ item: item, list: snImgList, group: 'sn', key: index }" -->
                <img class="houseimg" :src="$imgurl(item.imagePath,'agent400x300')">
                <div>
                  <span v-if="item.flag == 0 || item.flag == ''" class="fl">
                    <a @click="setTitPic(item)">设为标题图</a>
                  </span>
        ...
        <FormItem label="小区图">
          ...
          <Row>
            <div class="housePicList">
              <div class="housePicListItem" v-for="(item,index) in xqImgList" v-if="item.imageType == 3  && item.isDel !=1" :key="index">
                <img class="houseimg" :src="$imgurl(item.imagePath,'agent400x300')">
                <div>
                  <span class="fr">
        ...
        <!-- <FormItem v-else>
          <Button type="primary" @click="handleSubmit('form',1)">录入房源并发布</Button>
          <Button type="ghost" @click="handleSubmit('form',2)">录入为待发布房源</Button>
    ...
    getDetail() {
      houseApi
        ...
        .then(response => {
          ...
          this.fullImagesList = this.form.houseRentImages
        })
        .catch(error => {
      ...
      houseApi
        ...
        .catch(error => {
          if (error.response.data.code == 20423) {
            ...
            this.$Notice.error({
              title: "修改房源失败",
              desc: nodesc ? "" : error.response.data.code.data,
              duration: 2
            });
          }
        });
