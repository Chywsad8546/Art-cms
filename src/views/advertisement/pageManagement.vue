    ...
    <Col span="100">
      <Card>
        ...
        <a href="#" slot="extra" @click.prevent="addModeButton">
          <Icon type="plus-circled"></Icon>添加栏目
        </a>
    ...
    <Modal v-model="isTrueAddTag" width="360" @on-ok="addNewsChannel()">
      ...
      >
        <FormItem label="应用名称" prop="stationId">
          <Select
            v-model="addNewsChannelModal.stationId"
            :label-in-value="true"
  ...
  methods: {
    ...
    addNewsChannel() {
      this.$refs["addNewsChannelModalform"].validate(valid => {
        if (valid) {
          adapi
            .addPage(this.addNewsChannelModal)
            .then(response => {
              if (response.data.data > 0) {
