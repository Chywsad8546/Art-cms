      ...
      <TabPane label="内容">
       <Form ref="formValidate" :model="share" :label-width="80">
        <Row class="navdhName">
         ...
         <Col span="24">
            <h3>状态</h3>
            <CheckboxGroup v-model="share.secondDetailParam.saleStatusId">
                <Checkbox v-for="(item,index) in share.searchCondition.saleStatus" :key="index" :label="Number(item.value)">{{item.text}}</Checkbox>
            </CheckboxGroup>
            <!-- <RadioGroup v-model="share.area" type="button" size="small" @on-change="areaChange">
              <Radio v-for="(item,index) in share.searchCondition.area" :key="index" :label="item.value" style="margin-left:5px;margin-bottom:5px;">{{item.text}}</Radio>
            </RadioGroup> -->
...
export default {
    ...
    methods: {
        ...
        houseSave(){
            api.getNewHouseList(this.share.secondDetailParam).then(response=>{
    ...
    created: function () {
        ...
        api.getCityAllInfo({cityDomain:"bj"}).then(response=>{
            ...
            this.share.searchCondition = response.data.cityAllInfos.searchConditionData.newhouse;
