...
fs.open('./build/env.js', 'w', function(err, fd) {
    const buf = 'export const errortip  = true;export const xiangmu = "kefu";export default "development";export const qufencity = 0;';
    fs.write(fd, buf, 0, buf.length, 0, function(err, written, buffer) {});
});
fs.open('./build/router.build.hook.js', 'w', function(err, fd) {
    const buf = 'import router from \'@/router/split/corp.router\'\n' +
...
module.exports = merge(webpackBaseConfig, {
    ...
    devServer: {
        ...
        proxy: {
            ...
            '/v1.0.0': {
                // 目标服务器地址
                target: 'http://corp.test.toutiaofangchan.com/',
                //路径重写
